...
import { DEFAULT_MENU_DATA, parsePriceOptions, getDisplayPrice, PriceOption } from './data/menuData';

// ==========================================
// 📋 CONFIGURACIÓN DE LA PLANTILLA DEL MENÚ - D’ANDRY
// ==========================================
const RESTAURANTE_NAME = "D’ANDRY Cevichería";
const RESTAURANTE_SLOGAN = "Frescura del mar a tu mesa";
const WHATSAPP_NUMBER = "51955362055"; // WhatsApp oficial D’ANDRY
const FACEBOOK_URL = "https://facebook.com/search/top?q=Diandra%20Ramirez";
const MAPS_URL = "https://www.google.com/maps/search/?api=1&query=D%27ANDRY+Cevicher%C3%ADa";
const LOGO_FOOTER_PATH = "/logo.png";
const BANNER_PATH = "/logo.png";
const MARQUEE_TEXT = "🌊 FRESCURA DEL MAR A TU MESA • CEVICHES, CANGREJO Y SABORES QUE ENAMORAN • FRESCO, SABROSO Y ORIGINAL 🦀🦑🐙 • ";
// ==========================================
...
export default function App() {
  ...
  // State para la selección de opción de precio (Modal)
  const [priceModalDish, setPriceModalDish] = useState<Dish | null>(null);
  ...
  const handleAddClick = (dish: Dish) => {
    const options = parsePriceOptions(dish.precio);
    if (options.length > 0) {
      setPriceModalDish(dish);
    } else {
      addToCartDirect(dish.nombre, dish.precio);
    }
  };

  const addToCartDirect = (nombre: string, precio: string) => {
    setCart(prev => {
  ...
  const calculateTotal = () => {
    return cart.reduce((acc, item) => {
      // Intentar extraer el número de precio
      const matches = item.precio.match(/\d+(\.\d+)?/g);
      const num = matches ? parseFloat(matches[0]) : 0;
  ...
  return (
    <div className="max-w-md mx-auto bg-white min-h-screen relative shadow-2xl overflow-hidden flex flex-col font-sans">
      ...
      <div className="px-5 pt-4 pb-3">
        <div className="relative w-full rounded-3xl overflow-hidden shadow-xl aspect-[2/1] bg-gradient-to-br from-primary/10 to-secondary/15 flex flex-col items-center justify-center text-center p-2 border border-primary/20">
          <img src="/logo.png" alt="D'ANDRY Cevicheria Logo" className="w-full h-full object-contain p-2" />
        </div>
      </div>
      ...
      <main className="flex-1 overflow-y-auto pb-32 px-5">
          ...
          <section key={cat.id} id={`cat-${cat.id}`} className="mb-10 scroll-mt-28">
            ...
            <div className="grid grid-cols-2 gap-4">
              {cat.items.map((dish, idx) => {
                const display = getDisplayPrice(dish.precio);
                return (
                  <motion.div
                    key={idx}
                    whileHover={{ y: -4 }}
                    ...
                    <div className="p-4 flex flex-col flex-1">
                      ...
                      <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-50">
                        <div className="flex flex-col">
                          ...
                          <span className="font-dish font-extrabold text-primary text-[14px] leading-tight">
                            {display.texto || dish.precio}
                          </span>
                        </div>
                        <motion.button
                          whileTap={{ scale: 0.8 }}
                          onClick={() => handleAddClick(dish)}
                          className="w-8 h-8 bg-primary text-white rounded-full flex items-center justify-center shadow-md shadow-primary/20 transition-all duration-200 shrink-0 hover:bg-primary/90"
                        >
        ...
        <footer className="mt-8 pt-8 pb-10 border-t border-gray-200 flex flex-col items-center justify-center">
          ...
          <div className="w-36 h-36 mb-6 rounded-3xl bg-white p-2 shadow-lg border border-primary/20 flex items-center justify-center">
            <img src="/logo.png" alt="D'ANDRY Logo" className="w-full h-full object-contain" />
          </div>
        ...
        {priceModalDish && (
          ...
          >
            <motion.div
              ...
              className="bg-white w-full max-w-sm rounded-t-[2.5rem] sm:rounded-[2.5rem] p-6 shadow-2xl relative"
            >
              <button
                onClick={() => setPriceModalDish(null)}
                className="absolute top-5 right-5 w-9 h-9 bg-gray-100 rounded-full flex items-center justify-center text-gray-500 hover:bg-gray-200 transition-colors"
              >
                <X size={20} />
              </button>
              ...
              <div className="mb-4 pr-8">
                ...
                <h3 className="font-dish font-bold text-dark text-lg leading-snug">
                  {priceModalDish.nombre}
                </h3>
              </div>

              <p className="text-xs text-gray-500 mb-5">
                Elige la porción que deseas agregar a tu pedido:
              </p>

              <div className="space-y-3 mb-4">
                {parsePriceOptions(priceModalDish.precio).map((opt, i) => (
                  <motion.button
                    key={i}
                    whileHover={{ scale: 1.01 }}
                    whileTap={{ scale: 0.98 }}
                    onClick={() => {
                      addToCartDirect(`${priceModalDish.nombre} (${opt.etiqueta})`, opt.precio);
                      setPriceModalDish(null);
                    }}
                    className="w-full bg-gradient-to-r from-primary/5 to-primary/10 hover:from-primary/15 hover:to-primary/20 border border-primary/20 p-4 rounded-2xl flex items-center justify-between transition-all duration-200 group text-left"
                  >
                    <div className="flex flex-col">
                      <span className="font-dish font-bold text-dark text-sm group-hover:text-primary transition-colors">
                        {opt.etiqueta}
                      </span>
                    ...
                    <div className="flex items-center gap-2">
                      <span className="font-dish font-extrabold text-primary text-base">
                        {opt.precio}
                      </span>
